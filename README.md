...
| [`usage-report`](usage-report/SKILL.md) | Generate a Claude Code token-usage and cost report from local session logs (`~/.claude/projects/*/*.jsonl`). Writes a markdown dashboard to `/tmp/`; optionally delivers it via a configured messaging channel. (Named `usage-report` to avoid clashing with Claude Code's built-in `/usage`.) |
